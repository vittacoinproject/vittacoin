...
#if defined(NDEBUG)
# error "Vittacoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x6b712413539c965a5888236a98d57cdd5c58a0a6ec52b870e42c26ba85f23b84");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Vittacoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Vittacoin Signed Message:\n";
...
{
    // Vittacoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 440000); // Vittacoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Vittacoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Vittacoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Vittacoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Vittacoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x9b8ca397d0b71560e8ea1c9d680e25f432bc7da16145fbbd37dbd8ad8f96e69c");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "leon builds vittacoin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50000 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("047e56f710e08abb3490d8a47d39b908d847cc27758a8fdad83bb2b170433f761e935cee95bcb50f0710a2f97355d01a47db7572ce5aac633bf031b0bf5066a931") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1572433009;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2089412597;
        if (fTestNet)
        {
            block.nTime    = 1572433032
;
            block.nNonce   = 387454841;
        }
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x2aece4e60d92bdb785b80a0afc868643e3eece1cdb91f40db94f9fc4690d5c1b"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfe, 0xc3, 0xb7, 0xdc }; // Vittacoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("VittacoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("VittacoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static VittacoinMiner(CWallet *pwallet)
{
    printf("VittacoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("vittacoin-miner");
    ...
    try { loop {
        ...
        printf("Running VittacoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("VittacoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&VittacoinMiner, pwallet));
}
